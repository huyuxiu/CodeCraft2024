...
void distributeRobots(){
	for(int b = 0; b < maxBlockId; b++){ //遍历每个连通块
		...
		std::unordered_map<int, std::pair<int, int>> class_robot_number; //类id:类机器人数, 是否已满足硬需求（最少分配一个）
		for(int ber:berthss){
			class_area[berth[ber].getClassId()] += berthArea[ber];
			total_area += berthArea[ber];
		}
		for(auto p:class_area){
			int res = std::min(std::max(int(robot_number * p.second / total_area), 1), rest_robot_number); //分配给该类的机器人数量
		...
		for(auto [cid, i]:class_robot_number){
			if(i.first > 0){
				...
				if(!i.second) must_class.insert(i.first);
			}
		}

		//先分给硬需求类（类选机器人
		if(must_class.size()){
			std::unordered_map<int, std::pair<int, int>> res; //类id:机器人id,距离
			for(auto i:free_robots){
				...
				if(!res.count(class_id) || dist < res[class_id].second) res[class_id] = {i, dist};
			}
			for(auto p:res){ //分配竞争到硬需求类的机器人到类里
				robot_in_class[p.first].push_back(p.second.first);
				robot[p.second.first].setClassId(p.first);
				free_robots.erase(p.second.first);
				if(free_class.count(p.first)){
					free_class[p.first] --;
					if(free_class[p.first] == 0) free_class.erase(p.first);
				}
			}
		}
		if(!free_robots.size()) continue;
		...
		if(free_class.size()){
			...
			for(auto i:free_robots){ //机器人离哪个类近
				...
				if(res1[class_id].size() < free_class[class_id]){
